...
let assert_stream_has_exactly stream messages =
    ...
    let read () =
        Reader.read_one_message_bin_prot transport.reader Bin_prot.Type_class.bin_reader_string
...
let ``Reader can parse [bin_prot]ed messages and use [read_one_message_bin_prot]`` () =
    ...
    let stream =
        create_stream_with_messages (List.map string_to_bin_prot_array messages)
...
let ``Reader errors if size too large`` () =
    ...
    let result =
        Reader.read_forever transport.reader (fun (_: byte []) -> failwith "Should not reach this code") (fun () -> ())
...
let ``The reader calls 'on_end_of_batch' after any data from the stream`` () =
    ...
    test_num_batches Handler_result.Continue total_num_bytes (Error Reader.Error.End_of_stream)
...
let ``Writes don't work after the writer is closed`` () =
    ...
    let result =
        Writer.send_bin_prot writer Bin_prot.Type_class.bin_writer_string message
    ...
    assert_stream_has_exactly (new System.IO.MemoryStream(stream.BufferAtClose())) [ message ]
