...
let async_choose f1 f2 =
    ...
    Thread.spawn_and_ignore "async_choose thread 1" (fun () -> ignore (f1 () |> result.TrySetResult: bool))

    Thread.spawn_and_ignore "async_choose thread 2" (fun () -> ignore (f2 () |> result.TrySetResult: bool))

    result.Task.Result

[<SealedAttribute>]
type t(writer_end: MemoryStream Sequencer.t, reader_end: MemoryStream Sequencer.t, time_source: Time_source.t) =
    inherit Stream()
...
let make_pair time_source =
    ...
    new t (host_a_buffer, host_b_buffer, time_source), new t (host_b_buffer, host_a_buffer, time_source)
