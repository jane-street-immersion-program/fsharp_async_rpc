...
let send_pipe_initial_response test_connection query_id response bin_writer_error =
    Test_connection.send_response
        test_connection
        (Stream_initial_message.bin_writer_t Bin_prot.Type_class.bin_writer_unit bin_writer_error)
        { id = query_id
          data =
            Ok(
                { unused_query_id = -1L
...
let send t test_connection =
    ...
    | Initial { bin_writer_error = bin_writer_error
                query_id = query_id
                response = response } -> send_pipe_initial_response test_connection query_id response bin_writer_error
