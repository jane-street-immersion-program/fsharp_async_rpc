...
let ``Connection errors out on invalid handshake response`` () =
    ...
    Transport.Writer.send_bin_prot_exn test_transport.writer Bin_prot.Type_class.bin_writer_string "garbage"
...
let expect_query test_connection =
    Transport.Reader.For_testing.consume_one_transport_message
        ...
        [ Bin_prot_reader.expect Message.bin_reader_nat0_t (Message.t.Query query_with_length_only)
          Bin_prot_reader.expect Bin_prot.Type_class.bin_reader_string query.data ]

let dispatch_query connection response_handler =
    Connection.dispatch connection response_handler Bin_prot.Type_class.bin_writer_string query
...
let roundtrip_dispatch_and_response test_connection connection =
    ...
    let response_handler (_: Bin_prot.Nat0.t Response.t) (_: Bin_prot.Common.buf) (_: Bin_prot.Common.pos_ref) =
    ...
    Test_connection.send_string_response test_connection { id = query.id; data = Ok "arbitrary" }
...
let ``Connection sends heartbeats after fixed period`` () =
    ...
    for (_: int) in 1..10 do
        ...
        Test_connection.expect_message test_connection Message.bin_reader_nat0_t Message.t.Heartbeat
...
let ``Connection dies if no data after fixed timeout period`` () =
    ...
    Assert.MatchesIgnoringLines("Close_started.*No heartbeats received for", Connection.open_state connection)
...
let ``Connection can handshake, send queries, and handle hard-coded responses`` () =
    ...
    let response_with_length_only =
        Response.map_data response Bin_prot.Size.bin_size_string

    let responses_to_send = 10

    let responses_received = ref 0

    let wait_for_responses n =
        spin_until (fun () -> !responses_received >= n)
...
let ``Closing the connection closes the stream and closes outstanding queries`` () =
    ...
    Assert.MatchesIgnoringLines("Error.*Close_started.*By_user", dispatch_query connection None)
...
let ``Closing the stream from the remote side causes connection close (via reader)`` () =
    ...
    Assert.MatchesIgnoringLines("Error.*Close_started.*reader loop finished", dispatch_query connection None)
...
let ``Write errors also cause connection to close`` () =
    ...
    Assert.MatchesIgnoringLines("Error.*Close_started.*Writer stopped.*test error", dispatch_query connection None)
