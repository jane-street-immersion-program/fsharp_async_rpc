...
let ``Can send RPCs and pass responses to handler`` () =
    ...
    let rpc =
        Rpc.create {| name = "test-rpc"; version = 1 |} Bin_prot.Type_class.bin_string Bin_prot.Type_class.bin_string
    ...
    let received_queries =
        List.map (fun i -> i, parse_string_query (Test_connection.reader test_connection)) dispatch_indices
...
let ``Can send pipe RPCs and pass responses to handler`` () =
    ...
    let response_data query_i response_i =
        sprintf "response-%d-%d" query_i response_i
        ...
        Pipe_rpc.dispatch_iter
            ...
            (fun result ->
                if !received_responses_this_query = responses_per_query then
                    ...
                    Assert.AreEqual((Pipe_message.Update(response_data query_i !received_responses_this_query)), result)
    ...
    let received_queries =
        List.map (fun i -> i, parse_pipe_query (Test_connection.reader test_connection)) dispatch_indices
    ...
    |> List.transpose
    |> Option.get
    |> List.iter (List.iter (fun pipe_response -> Pipe_response.send pipe_response test_connection))
...
let ``Error in initial pipe response results in an inner error`` () =
    ...
    let query_id =
        dispatch_and_get_id test_connection connection initial_response.SetResult Assert.Unreached
...
let ``Invalid initial pipe response results in an outer error`` () =
    ...
    | Error e -> Assert.That(e.ToString(), Does.Match("Exn .* streaming_rpc initial_response un-bin-io'ing"))
...
let ``Error pipe update response results in an update error`` () =
    ...
    | Pipe_message.Closed_from_error e -> Assert.That(e.ToString(), Does.Match("Connection_closed"))
...
let ``Invalid pipe update response results in an update error`` () =
    ...
    | Pipe_message.Closed_from_error e ->
        Assert.That(e.ToString(), Does.Match("Exn .* streaming_rpc response header un-bin-io'ing"))
    | e -> failwithf "Unexpected update_response: %A" e
