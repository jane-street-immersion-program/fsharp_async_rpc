...
let create_exn (protocol: 'a Known_protocol.t) (args: {| supported_versions: int64 list |}) =
    let length = 1 + args.supported_versions.Length

    if length > Protocol_version_header.max_len then
        failwithf "List is too large. (length: %d) (max_len: %d)" length Protocol_version_header.max_len
