                      ...
                      | Ok initial_msg ->
                          (match initial_msg.initial with
                           ...
                           | Ok initial ->
                               ...
                               state.state <-
                                   State.Writing_updates(initial_handler.rpc.bin_update_response.reader, handler)
...
module Pipe_rpc =
    type ('query, 'response, 'error) t = T of Streaming_rpc.t<'query, unit, 'response, 'error>

    let create args bin_query bin_response bin_error =
        Streaming_rpc.create args bin_query Bin_prot.Type_class.bin_unit bin_response bin_error
        |> T
