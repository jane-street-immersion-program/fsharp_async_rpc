...
let expect_version version expected =
    if version = expected then
        ...
        Or_error.Error.format "Negotiated unexpected version: %d. Expected version: %d" version expected
...
let do_handshake (transport: Transport.t) protocol =
    Result.let_syntax {
        ...
        | Known_protocol.Krb_test_mode ->
            ...
            do!
                Transport.Writer.send_bin_prot transport.writer Krb.Test_mode_protocol.Syn.bin_writer_t syn
                |> Transport.Send_result.to_or_error

            let! syn_result =
                Transport.Reader.read_one_message_bin_prot transport.reader Krb.Test_mode_protocol.Syn.bin_reader_t
      ...
      run integration tests against servers running in this test mode. *)
            ...
            do!
                Transport.Writer.send_bin_prot transport.writer Krb.Test_mode_protocol.Ack.bin_writer_t ack
                |> Transport.Send_result.to_or_error

            do!
                Transport.Reader.read_one_message_bin_prot transport.reader Krb.Test_mode_protocol.Ack.bin_reader_t
                |> Result.mapError (Transport.Reader.Error.to_error)
                |> Result.join
...
let do_handshake_with_timeout transport protocol =
    with_set_read_timeout (Transport.stream transport) (fun () -> do_handshake transport protocol)
        ...
        Sequencer.with_ t (fun t ->
            ...
            | None ->
                Transport.Handler_result.Stop(
                    Rpc_error.t.Unimplemented_rpc(query.tag, Rpc_error.Unimplemented_rpc.t.Version query.id)
                )
            | Some foo ->
                ...
                //start the thread here there is a risk of not being able to send an error 
                    async {Implementation.apply foo () query read_buffer read_buffer_pos_ref t.writer|> ignore}|>Async.Start
...
let open_state (T t) =
    Sequencer.with_ t (fun t -> t.open_state)
...
let heartbeat_now (T t) =
    Sequencer.with_ t (fun t ->
        ...
        if since_last_heartbeat > heartbeat_timeout then
            ...
            (match writer t with
             | Error close_reason -> Transport.Send_result.Close_started close_reason
             | Ok writer -> Transport.Writer.send_bin_prot writer Message.bin_writer_nat0_t Message.t.Heartbeat)
            |> Transport.Send_result.to_or_error)
...
let run_after_handshake (T t) reader =
    ...
    let reason =
        Transport.Close_reason.errorf "Connection reader loop finished: %A" result
...
let create
    ...
    implementations
    |> List.iter (fun implementation -> implementation_dict.Add(Implementation.rpc_info implementation, implementation))
    ...
    let create_and_handshake () =
        Result.let_syntax {
            ...
            | Error error ->
                Sequencer.with_ t (fun t -> cleanup t (Transport.Close_reason.errorf "Handshake failed"))
...
module For_testing =
    let create_wait_for_connection stream time_source protocol (args: {| max_message_size: int |}) =
        let wait_connection = TaskCompletionSource<_>()

        create stream time_source protocol {| max_message_size = args.max_message_size |} wait_connection.SetResult []
