...
module With_limit =
    ...
    let create t (args: {| max_message_size: int |}) =
        if args.max_message_size < 0 then
            Or_error.Error.format "Rpc_transport.With_limit.create got negative max message size: %A" args
        else
            Ok
                { t = t
...
module Header =
    ...
    let write_payload_length (writer: System.IO.BinaryWriter) payload_len = writer.Write(int64 (payload_len))
    ...
    let rec read_forever t on_message on_end_of_batch =
        let result =
            Result.let_syntax {
                ...
                let! () = read_bytes t.stream.t t.length_buffer Header.header_length on_end_of_batch
        ...
        read_forever
            ...
            (fun buf ->
                ...
                if !pos_ref <> buf.Length then
                    sprintf "message length (%d) did not match expected length (%d)" (!pos_ref) buf.Length
                    |> Error.other
                    |> Error
                    |> Handler_result.Stop
                else
            ...
            read_forever
                ...
                (fun buf ->
                    ...
                    if buf.Length <> !pos_ref then
                        failwithf "Buffer (length %d) was not fully consumed (consumed %d)" buf.Length !pos_ref
...
module Writer =
    ...
    let try_write writer (buffer: byte []) =
        try
            ...
            Ok()
        with
        | :? System.ObjectDisposedException -> Or_error.Error.format "Underlying stream was closed"
        | :? System.IO.IOException as e -> Or_error.Error.format "Got IO exception from writer: %A" e.Message
    ...
    let create stream (args: {| max_message_size: int |}) : t Or_error.t =
        Result.let_syntax {
            ...
            Thread.spawn_and_ignore "transport background writer" (background_writer_loop t queue_reader stream)
    ...
    module For_testing =
        let wait_for_flushed (T t) =
            ...
            Sequencer.with_ t (fun t ->
                match t.open_state with
                | Open_state.Open -> Blocking_queue.Writer.write t.to_send.t (Message.Flush signal)
